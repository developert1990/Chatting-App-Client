...
export const ChatScreen: React.FC<ChatScreenPropsType> = ({ socket }) => {
    ...
    const submitFunc = () => {
        ...
        chatData?.users.filter((user, index) => {
            if (user._id !== signinInfo._id) {
                ...
                socket?.emit("newNotification", user._id);
            }
    ...
    useEffect(() => {
        console.log("------------------타이핑 리씨브 ---------------------------")
        socket.on("receive typing", () => typingDotsRef.current?.classList.add("show"))
        socket.on("noContents", () => typingDotsRef.current?.classList.remove("show"))
    }, [socket.emit, socket, msgcontents])
